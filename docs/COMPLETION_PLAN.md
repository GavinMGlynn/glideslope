...
- [ ] **The Cesium cache still locks when the rendering tests run together.**
      Programs can now share one cache and each stores everything; one clean
      suite run of the ten is done, and nine are still to do. *Verification:
      the whole suite at `-j4` reports no locked cache, ten times over.*
