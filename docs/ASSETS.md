...
| Changes | Made by `tools/make_f15c.py` with `tools/fighter.py` and `tools/airliner.py`, whose docstrings list each change and why: the F-15C's weights, fuel and stores; lift and drag across the Mach range, and the drag past the flow separating; the F100-PW-220's thrust, its afterburner lit above 0.99, and its thrust with speed and height, fitted to the Standard Aircraft Characteristics' charts; and its centre of gravity, with the aerodynamic reference point, moved aft until the main wheels are 15 degrees behind it (Raymer's tipback angle), which brings its nose wheel off within 7 knots of T.O. 1F-15A-1's figure A3-6 (<https://archive.org/details/f-15-manual>, the flight manual recorded below for the checklists); and four airframe contacts either side of its keel, measured from its visual model by `tools/ground.py`, so that landed wheels-up it rests on its belly rather than rocking tip to tip. |
...
| Raymer, Aircraft Design: A Conceptual Approach, the tipback angle | A textbook | Where the F-35B's main wheels are, which nothing published gives: 15 degrees behind the centre of gravity seen from the ground |
...
| The F-35B's lift-off speed, and the ground roll to it | 189 KCAS and 2,008 ft at its `combat` loading. **Its stall is at 31 degrees of incidence**, with the engine holding up a third of its weight, so the rotation speed the stall would give, 141 knots, cannot be flown on a runway. Held at the take-off autopilot's ten degrees from 100 knots it left the runway at 172 - its minimum unstick speed at that attitude - and the lift-off speed is 1.1 times that, as FAR 25.107 has it at least 110 per cent of the minimum unstick speed. The ground roll is what the `takeoff_ground_roll` flight measures to it |
